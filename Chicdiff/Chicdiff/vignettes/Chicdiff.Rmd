...
Note that the diagnostic plots above are computed using only a small fraction of the full CD4 and Monocyte datasets. In real-world, genome-wide datasets the trends described below should be more pronounced. 

### Output diagnostic plots

Below we describe the expected properties of the diagnostic plots, showing whether the criteria for the covariates are applicable.

#### Decision boundary
```{r}
resultsPath <- file.path(dataPath, "CD4_Mono_results")

IHWdecisionBoundaryPlot <- readPNG(file.path(resultsPath,"IHWdecisionBoundaryPlot.png"))
...
#### Estimated weights
```{r}
IHWweightPlot <- readPNG(file.path(resultsPath,"IHWweightPlot.png"))
...
### Differential contact plots

If the option *plot* in the defchic.settings was marked as TRUE (that is by default set up to TRUE), chicdiff will produce for random plots from the top 100 interactions with the lowest weighted pvalue.
...
IHWweightPlot <- readPNG(file.path(resultsPath,"diffbaitPlot.pdf"))
grid::grid.raster(IHWweightPlot)
```
```{r}
library(Biobase)
resultsPath <- file.path(dataPath, "CD4_Mono_results")
openPDF(file.path(resultsPath,"diffbaitPlot.pdf"))
```
