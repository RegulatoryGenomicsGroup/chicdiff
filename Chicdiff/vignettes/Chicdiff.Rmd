...
Note that the diagnostic plots are computed using only a small fraction of the full CD4 and Monocyte datasets. In the real-world genome-wide datasets the trends described below should be more pronounced. 
...
```{r, warning=FALSE}
results <- readRDS(file.path(resultsPath, "test_results.Rds"))
countput <- readRDS(file.path(resultsPath, "test_countput.Rds"))
bmap <- fread(file.path(testDesignDir, "chr19_GRCh37_HindIII.baitmap"))

baits <- c(331172, 320562, 327700)
plotDiffBaits(output = output, countput = countput, baitmapfile = bmap, baits = baits)
```
...
To obtain indications of differential signals at individual fragments, Chicdiff provides the function  ```getCandidateInteractions()```.

FIXME: To finish the docs


```{r}

outCI <- getCandidateInteractions(chicdiff.settings = chicdiff.settings, output = output, peakFiles = peakFiles, pvcut = 0.05, deltaAsinhScore =1)
